...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	/**
	 * Reads names of the worksheets from a file, without parsing the whole file to a PHPExcel.class object
	 *
	 * @param 	string 		$pFilename
	 * @throws 	PHPExcel_Reader_Exception
	 */
	...
	public function load($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			...
			// add sheet to PHPExcel.class object
	...
	private function _readSummaryInformation()
	{
		...
		for ($i = 0; $i < $countProperties; ++$i) {
			...
			switch ($id) {
				...
				case 0x07:	//	Template
					//	Not supported by PHPExcel.class
					break;
	...
	private function _readStyle()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($isBuiltIn) {
				...
				// user-defined; not supported by PHPExcel.class
			}
		}
	}
	...
	private function _readPane()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($this->_frozen) {
				...
				// unfrozen panes; split windows; not supported by PHPExcel.class core
			}
		}
	}
